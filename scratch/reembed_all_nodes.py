...
async def ollama_embed_batch(session: aiohttp.ClientSession, texts: list[str]) -> list[list[float]]:
...
async def reembed_label(driver, session, label: str, dry_run: bool = False, batch_size: int = 50):
    ...
    if dry_run:
        print(f"   🏴 DRY RUN — would embed {total} nodes in {((total + batch_size - 1) // batch_size)} batches")
    ...
    for batch_idx, (batch_texts, batch_nodes) in enumerate(zip(batches, node_batches)):
        try:
            ...
            for node, emb in zip(batch_nodes, embeddings):
                if len(emb) != NEW_DIM:
                    print(f"   ⚠️  Unexpected embedding dim {len(emb)} for node {node['eid'][:12]}...")
                    continue
        ...
        print(f"   [{batch_idx+1}/{len(batches)}] {updated}/{total} ({pct:.0f}%) — {rate:.0f} nodes/s — ETA {eta:.0f}s", end="\r")
...
async def main():
    parser = argparse.ArgumentParser(description="Re-embed old 768-dim nodes with Qwen 4B 2560-dim embeddings")
    parser.add_argument("--dry-run", action="store_true", help="Count nodes but don't embed")
    parser.add_argument("--labels", nargs="+", default=["Entity", "Fact"], help="Labels to re-embed (default: Entity Fact)")
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(f"{OLLAMA_URL}/api/tags", timeout=aiohttp.ClientTimeout(total=10)) as resp:
                tags = await resp.json()
                models = [m["name"] for m in tags.get("models", [])]
                if OLLAMA_MODEL not in models:
                    print(f"⚠️  Model '{OLLAMA_MODEL}' not in {models}. Available: {models}")
                else:
    ...
    async with aiohttp.ClientSession() as http_session:
        for label in args.labels:
            await reembed_label(driver, http_session, label, dry_run=args.dry_run, batch_size=args.batch_size)
